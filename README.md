...
Пароли читаются из ENV `SITE_PASSWORD_DAD` (по умолчанию `1133`, режим редактора) и `SITE_PASSWORD_MOM` (по умолчанию `0808`, режим просмотра). Пустая строка в ENV (например незаполненный GitHub Secret) трактуется как «нет значения» и подставляется дефолт.
...
3. В репозитории на github.com:
   ...
   - Если secret'ы не задать — будут дефолты `1133` (папа) и `0808` (мама). **Не оставляй пустое значение** — эту проблему код обрабатывает (пустое = дефолт), но лучше либо задать значение, либо удалить secret целиком
